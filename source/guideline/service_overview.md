...
| Geometry | See technical design | 0..1 | Geometry condition for geolocated information objects. This can be used to filter on geometric shapes (e.g. filter AtoN information S-201 datasets by a bounding box). |
...
The operation follows a request-callback paradigm (REQUEST_CALLBACK), in which the consumers register their interest in a set of AtoN information and receive updates whenever one of the corresponding S-201 datasets change. The subscription can be cancelled through the Remove Subscription operation or can be limited by specifying a subscription period.
